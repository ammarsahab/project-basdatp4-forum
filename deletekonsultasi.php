...
if(empty($konsultasi)){
    header('location:konsultasi.php?issue='.$issue.'');
    exit;
}
...
$konsultasidata=pg_prepare("konsultasidata", 'SELECT * from konsultasi where konsultasi_id=$1');
$konsultasidata=pg_execute("konsultasidata",array($konsultasi));
$kdata=pg_fetch_array($konsultasidata);
...
if (isset($_POST['Delete'])) {
    $stmt = pg_prepare('carikonsultasi','SELECT * FROM konsultasi WHERE konsultasi_id=$1');
    $stmt = pg_execute('carikonsultasi',array($konsultasi));
    $sm = pg_num_rows($stmt);
    if ($sm==0) {
        echo 'Tidak ada konsultasi dengan id tersebut';
        exit;
    }
    $delpost = pg_prepare('delpost','DELETE FROM post WHERE konsultasi_id=$1');
    $delpost = pg_execute('delpost',array($konsultasi));
    $delkonsultasi = pg_prepare('delkonsultasi','DELETE FROM konsultasi WHERE konsultasi_id=$1');
    $delkonsultasi = pg_execute('delkonsultasi',array($konsultasi));
    header('location: consul.php?issue='.$issue.'&status=deletekonsultasi');
    exit;
    }?>
<!DOCTYPE html>
<head>
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <a class="nav-link dropdown-toggle" href=# id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
          ...
          <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
            <li><a class="dropdown-item" href="konsultasis.php?issue=<?php echo $issue;?>">Survei</a></li>
            <li><a class="dropdown-item" href="konsultasis.php?issue=<?php echo $issue;?>">Forum</a></li>
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST"> 
    ...
    <input type="number" title="konsultasi" name="konsultasi" value=<?php
    if(isset($konsultasi)){
        echo $konsultasi;
    }else{
        echo $_GET['konsultasi'];
    }
    ?>></div>
    <h2>Apakah anda yakin ingin delete konsultasi <?php echo $kdata['konsultasi_id'];?>?
        <p>
            <button class="btn btn-success" type="submit" value="Delete" name="Delete">Delete</button>
            <a class="btn btn-danger" href="consul.php?issue=<?php echo $_GET['issue'];?>" role="button">Tidak.</a>
       </p>
    </form>
</div>
</body>
