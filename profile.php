<?php
// Initialize the session
session_start();
 
// Check if the user is logged in, if not then redirect him to login page
if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true){
    header("location: login.php");
    exit;
}
if(isset($_GET['user'])){
    $id=$_GET['user'];
}else{
    $id=$_SESSION['id'];
}
include('connect.php');
$user=pg_prepare("user", 'SELECT user_id, username, email from users where user_id=$1');
$user=pg_execute("user",array($id));
$profile=pg_fetch_array($user);
$facilcheck=pg_prepare("facilcheck", 'SELECT facil_id from facilitator where facil_id=$1');
$asncheck=pg_prepare("asncheck", 'SELECT asn_id from asn where asn_id=$1');
$facilcheck=pg_execute("facilcheck",array($id));
$asncheck=pg_execute("asncheck",array($id));
if(pg_num_rows($facilcheck)==1 && pg_num_rows($asncheck)==1){
    $status="Fasilitator, ASN";
    }elseif(pg_num_rows($facilcheck)==1 && pg_num_rows($asncheck)==0){
    $status= "Fasilitator";
    }elseif(pg_num_rows($facilcheck)==0 && pg_num_rows($asncheck)==1){
    $status= "ASN";
    }else{$status="User";}
?>
<!DOCTYPE HTML>
<head>
    <meta charset="utf-8">
    <link rel="stylesheet" href="bootstrap-5.0.1-dist/css/bootstrap.css"/>
    <script src="bootstrap-5.0.1-dist/js/bootstrap.bundle.js"></script>
    <title>Home</title>
<body>
<header>
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <div class="container">
    <a class="navbar-brand" href="#">Forum</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="home.php">Home</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="issues.php">Isu</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="users.php">Pengguna</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="profile.php">Profil</a>
        </li>
        <li class="nav-item dropdown"> 
        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            Akun
          </a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
            <li><a class="dropdown-item" href="logout.php">Log out</a></li>
            <li><a class="dropdown-item" href="resetpassword.php">Reset sandi</a></li>
          </ul>
        </li>
      </ul>
    </div>
  </div>
</nav>
</header>
<div class="container">
<div class="jumbotron">
<h2 class="text-center">Profil Pengguna</h2>
<div class="card bg-light text-black">
<div class="card-body">
<h3>Username</h3>
<h3><span style="font-weight:normal"><?php echo $profile['username']?></span></h3>
<br>
<h3>Email</h3>
<h3><span style="font-weight:normal"><?php echo $profile['email']?></span></h3>
<br>
<h3>Status</h3>
<h3><span style="font-weight:normal"><?php echo $status?></span></h3>
</div>
</div>
</body>
</html>
