...
if(empty($post)){
    header('location:posts.php?issue='.$issue.'&topic='.$topic.'');
    exit;
}
//cari data topik, isu, post
...
$postdata=pg_prepare("postdata", 'SELECT * from post where post_id=$1');
$postdata=pg_execute("postdata",array($post));
$pdata=pg_fetch_array($postdata);
...
if (isset($_POST['Delete'])) {
    $stmt = pg_prepare('caripost','SELECT * FROM post WHERE post_id=$1');
    $stmt = pg_execute('caripost',array($post));
    $sm = pg_num_rows($stmt);
    if ($sm==0) {
        echo 'Tidak ada post dengan id tersebut';
        exit;
    }
    $delpost = pg_prepare('delpost','DELETE FROM post WHERE post_id=$1');
    $delpost = pg_execute('delpost',array($post));
    header('location: posts.php?issue='.$issue.'&topic='.$topic.'&status=deletepost');
    exit;
    }?>
<!DOCTYPE html>
<head>
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST"> 
    ...
    <input type="number" title="post" name="post" value=<?php
    if(isset($post)){
        echo $post;
    }else{
        echo $_GET['post'];
    ...
    ?>></div>
    <h2>Apakah anda yakin ingin delete post <?php echo $pdata['post'];?>?
        <p>
